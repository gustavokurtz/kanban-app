...
import { PlusCircle, Clock, X, RefreshCw, LogOut } from "lucide-react"
...
export default function Tasks() {
  ...
  useEffect(() => {
    ...
    if (!token) {
      ...
      // Validar o token no backend (opcional)
      const validateToken = async () => {
        try {
          // Fazer uma requisição simples para verificar se o token é válido
          // Pode ser qualquer endpoint protegido, como obter dados do usuário
          await apiRequest('https://nestauth-api-production.up.railway.app/users');
  ...
  useEffect(() => {
    // Só carregar tarefas se estiver autenticado
  ...
  // Se ainda estamos verificando autenticação, mostrar um spinner
        ...
        <div className="backdrop-blur-md bg-white/10 rounded-xl p-5 shadow-lg">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-0">
            ...
            <div className="flex flex-col md:flex-row gap-2 w-full md:w-auto">
              <Button 
                className="w-full md:w-auto bg-gradient-to-r from-violet-500 to-purple-600 hover:opacity-90 text-white transition-all duration-300 border-0 rounded-lg shadow-lg hover:shadow-violet-500/20 flex items-center justify-center gap-2"
              ...
              <Button 
                className="w-full md:w-auto bg-gradient-to-r from-pink-500 to-red-500 hover:opacity-90 text-white transition-all duration-300 border-0 rounded-lg shadow-lg hover:shadow-red-500/20 flex items-center justify-center gap-2"
